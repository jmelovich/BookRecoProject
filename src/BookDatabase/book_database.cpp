...
void mergeSort(vector<BookEntry>& books){
    int left = 0;
...
void mergeSortHelper(vector<BookEntry>& books, int left, int right){
    if (left < right){
...
public:
    BookDatabase_Type2(const std::string& file_path, const int depth = -1) : BookDatabase(file_path) {
        bool loadedData = loadDataFromDisk(file_path, depth);
        if (!loadedData) {
            throw std::runtime_error("Failed to load data from disk");
        }
...
// Class for loading in and sorting data with Quick Sort
class BookDatabase_Type3 : public BookDatabase {
public:
    BookDatabase_Type3(const std::string& file_path, const int depth = -1) : BookDatabase(file_path) {
        bool loadedData = loadDataFromDisk(file_path, depth);
        if (!loadedData) {
            throw std::runtime_error("Failed to load data from disk");
        }
    ...
    bool loadDataFromDisk(const std::string& file_path, const int depth = -1) override {
        ...
        book_data_ = Quick_Sort(book_data_);
...
//Class for loading in data and sorting with Merge Sort
class BookDatabase_Type4 : public BookDatabase {
public:
    BookDatabase_Type4(const std::string& file_path, const int depth = -1) : BookDatabase(file_path) {
        bool loadedData = loadDataFromDisk(file_path, depth);
        if (!loadedData) {
            throw std::runtime_error("Failed to load data from disk");
        }
...
PYBIND11_MODULE(book_database_cpp, m) {
    ...
    py::class_<BookDatabase_Type3, BookDatabase, std::shared_ptr<BookDatabase_Type3>>(m, "BookDatabase_Type3")
        .def(py::init<const std::string&>())
        .def(py::init<const std::string&, int>());
    py::class_<BookDatabase_Type4, BookDatabase, std::shared_ptr<BookDatabase_Type4>>(m, "BookDatabase_Type4")
        .def(py::init<const std::string&>())
        .def(py::init<const std::string&, int>());
}